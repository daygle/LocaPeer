...
object MarkerIconFactory {
    ...
    fun create(context: Context, displayName: String, isOverdue: Boolean, isSos: Boolean, pinColor: String = "", isSelected: Boolean = false): BitmapDrawable {
        ...
        val bitmap = createBitmap(sizePx + 8, sizePx + 16, Bitmap.Config.ARGB_8888)
        bitmap.applyCanvas {
            val cx = (sizePx + 8) / 2f
            ...
            drawCircle(cx, cx + 2, radius, shadowPaint)

            // Circle fill
            val fillPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = fillColor }
            drawCircle(cx, cx, radius, fillPaint)
            ...
            drawCircle(cx, cx, radius - 1, borderPaint)
            ...
            drawText(initial, cx, cx - bounds.exactCenterY(), textPaint)
